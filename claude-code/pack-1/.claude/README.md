...
- `skills/` - Reusable skill cards for common conversion tasks.
...
## Skill format
Each skill in `.claude/skills/` must include YAML frontmatter metadata with `name` and `description` (optionally `metadata.short-description`) to support concise discovery and loading.
